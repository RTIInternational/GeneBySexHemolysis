...
params.interaction_col = "The column number for the interaction term"
params.interaction_covar = "The label for the interaction covariates, e.g. for GxG this would be the rsID"
...
process do_probabel_analysis{
	...
	/share/storage/REDS-III/common/software/${params.method} \
        ...
        --interaction "${params.interaction_col}" \
    	--out "${geno.baseName}"
	"""
}
...
process calc_chi_p_or{
	...
	"""
	/share/storage/REDS-III/common/software/R/calculate_2df_stats_from_probabel_results_v1.R \
		--in "${result}" \
		--out "${result.baseName}.stats" \
        --interaction_covar "${params.interaction_covar}"
	"""
}
